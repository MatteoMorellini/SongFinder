...
def _hash_triplet(f_anchor: int, f_target: int, dt: int,
    ...
    if mostrare: 
        print(f"Quantized: f_anchor={fa}, f_target={fb}, dt={dt}")
        print((fa << 22) | (fb << 12) | dt)
...
def build_hashes(
    ...
    for i in range(n_peaks):
        ...
        for (_, f_b, _, dt) in strongest:
            ...
            if not isinstance(h, np.int64):
                _hash_triplet(f_a, f_b, dt, fuzz=fuzz, mostrare=True)
                print(f"Hash value: {h}")
                print(f"Hash type: {type(h)}")
                print(f_a, f_b, dt)
                raise ValueError("Hash is not np.int64")
...
def plot_spectrogram_and_save(signal, sample_rate, output_path: Path, bands):
    ...
    print(fingerprints[10000:10050])

...
def recognize_song():
    ...
    xs = [p[0] for p in matching_pairs[1]]
    ys = [p[1] for p in matching_pairs[1]]
    ...
    offsets = [t_db - t_q for (t_q, t_db) in matching_pairs[1]]

    plt.figure(figsize=(8, 3))
    plt.hist(offsets, bins=np.arange(min(offsets), max(offsets)+0.1, 0.1))
    plt.xlabel("Offset (s)")
    plt.ylabel("Count")
